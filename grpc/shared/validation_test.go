...
import (
	bidv1alpha1 "buf.build/gen/go/astria/execution-apis/protocolbuffers/go/astria/auction/v1alpha1"
	"bytes"
	"crypto/ecdsa"
	"crypto/ed25519"
...
func TestUnmarshallAllocationTxs(t *testing.T) {
	...
	tests := []struct {
		description    string
		allocation     *bidv1alpha1.Allocation
		prevBlockHash  []byte
...
func TestUnbundleRollupDataWithDuplicateInvalidAllocations(t *testing.T) {
	...
	invalidPayload := &bidv1alpha1.Bid{
		Fee:                      100,
	...
	invalidAllocation := &bidv1alpha1.Allocation{
